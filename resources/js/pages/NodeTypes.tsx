            ...
            >
                <form id="nt-form" onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-200 sm:pt-6">
                        <input type="checkbox" checked={form.is_active} onChange={(e) => setForm({ ...form, is_active: e.target.checked })} className="h-4 w-4 rounded border-gray-300 dark:border-night-300 text-brand-600 dark:text-brand-400 focus:ring-brand-500" />
                        {t('common.active')}
                    </label>
                </form>
            </Modal>

    ...
    return (
        <Card className="p-5">
            ...
            <p className="mt-1 text-3xl font-bold text-gray-900">
                {value}
                <span className="ml-0.5 text-base font-semibold text-gray-400">{suffix}</span>
            </p>
        </Card>
    );
}
            ...
            <CardBody className="space-y-4">
                <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                        ...
                        <p className="mt-1 text-xl font-bold text-gray-900">{book.read_percent}%</p>
                    </div>
                    <div>
                        <p className="text-xs font-medium text-gray-500">Listened</p>
                        <p className="mt-1 text-xl font-bold text-gray-900">{book.listen_percent}%</p>
                    </div>
                    <div>
                ...
                <div>
                    <div className="mb-1 flex items-center justify-between text-xs">
                        <span className="font-medium text-gray-500">Overall reading</span>
                        <span className="font-bold text-brand-700">{book.read_percent}%</span>
                    </div>
                    ...
                    <div className="space-y-2.5">
                        {book.chapters.map((ch) => (
                            <div key={ch.id} className="flex flex-wrap items-center gap-2">
                                <span className="w-2/5 truncate text-sm text-gray-700">{ch.name}</span>
                                <div className="min-w-[70px] flex-1">
                                    <PercentBar value={ch.activity_percent} />
                                </div>
                                <span className="w-9 text-right text-xs font-semibold text-gray-600">{ch.activity_percent}%</span>
                                {ch.exam ? (
                                    ch.exam.attempted ? (
                ...
                {attemptedChapters.length === 0 && (
                    <p className="text-xs text-gray-400">No exams attempted for this book yet.</p>
                )}
            </CardBody>
        </Card>
    );
...
export default function ProgressView({ report, emptyTitle }: { report: ProgressReport | null; emptyTitle: string }) {
    if (!report) {
        return <Card><CardBody><div className="py-16 text-center text-sm text-gray-500">No progress data yet.</div></CardBody></Card>;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Card className="p-5">
                    <p className="text-sm font-medium text-gray-500">Most read chapter</p>
                    {report.most_read_chapter ? (
                        <div className="mt-1">
                            <p className="truncate text-base font-bold text-gray-900">{report.most_read_chapter.chapter_name}</p>
                            <p className="truncate text-xs text-gray-500">
                                {report.most_read_chapter.book_name} · {report.most_read_chapter.percent}%
                            </p>
                        </div>
                    ) : (
                        <p className="mt-1 text-sm text-gray-400">No chapters read yet.</p>
                    )}
                </Card>
            </div>

            {report.books.length === 0 ? (
                <Card><CardBody><div className="py-16 text-center text-sm text-gray-500">{emptyTitle}</div></CardBody></Card>
            ) : (
